...
import json
import pprint
from HarmonyClient import HarmonyClient
import Harmony
import sleekxmpp
import sqlite3
...
def createTable():
    queryCurs.execute('''CREATE TABLE userinfo
...
def updateInfo(key,expires):
    queryCurs.execute("UPDATE userinfo SET key = ?, expires = ?", (key,expires))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def get_data(self, data):
        self.send_response(302)
        createDb = sqlite3.connect('key.db')
        queryCurs.execute('SELECT key FROM userinfo')
